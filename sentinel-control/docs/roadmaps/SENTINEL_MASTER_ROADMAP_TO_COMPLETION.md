...
session after safe product cards were already captured in the world model. V6
then proved Cloak-backed search can complete with a material receipt, but the
next extraction mission received empty safe context and blocked with
`browser_session_missing_or_closed`. The fixes now make mission request IO
long-path aware, make env-configured product browser actions use the
Cloak-first real browser engine factory, make high-level search self-open
Cloak/session, route recoverable browser search failures with visible cards into
...
dead extraction. The follow-up fix now routes first-turn extract/verify without
browser context into a live search first. This makes the current product truth
explicit:
...
first-turn extract without browser context routes to search = FIXED LOCALLY
verify_extraction = material proof receipt for product proof
local product browser fixture = explicit fake/local profile only
current real-attempt readiness = GO_AFTER_FIRST_TURN_EXTRACT_ROUTING_FIX
...
through V7 as valid failed proofs and prepares the next single real attempt:
