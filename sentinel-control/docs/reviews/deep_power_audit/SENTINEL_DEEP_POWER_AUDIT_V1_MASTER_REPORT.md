...
| Cognitive OS North Star and Browser Cortex Search Quality V1 | Implemented candidate, local proof only | `16facc2afa46c8f1d9a60a824396db93b22b19bf` | Locks Sentinel as a cognitive operating system, then upgrades `BrowserEnvironmentState` into an evidence/confidence/freshness/source/uncertainty-bearing cognitive state and adds search materiality receipts so input filling alone is not counted as successful search |
| Browser Cortex Quality Corpus and Search Understanding Gate | Implemented candidate, local corpus proof only | local commit containing this report | Adds a frozen 24-case deterministic corpus, 20-task/5-site holdout manifest, counterfactual BrowserEnvironmentState decision-consumption proof, search progress states, fill-only false-success evaluator, semantic result/entity graph, and safe state-field redaction correction; real holdout quality remains unrun |
| Browser Cortex deterministic corpus execution and baseline | Valid failed deterministic baseline | local commit containing this report | Executes all 24 frozen deterministic cases through the product browser spine, records permanent per-case baseline JSON, proves replay no-react and no fill-only false success, and exposes remaining deterministic gaps in URL-query/no-result completion, localized search control selection, stale-control recovery, recovery success, and uncertainty accuracy |
