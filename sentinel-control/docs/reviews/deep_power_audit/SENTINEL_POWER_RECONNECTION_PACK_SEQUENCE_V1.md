...
| Power friction cut: first stupid blockers | `POWER_FRICTION_CUT_PACK_1_REMOVE_STUPID_BLOCKERS_V1` | implemented candidate, not real-provider/browser product-proven | pending local commit | Cuts the first 5F blocker cluster: visible cards beat stale open/search recommendations, safe ambiguous intent routes to extract/verify/finish, hidden/disabled refs recover, secret refs remain hard stops |
