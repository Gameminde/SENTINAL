...
```text
SENTINEL_MONSTER_RUNTIME_OBJECTIVE_LOCK_V1
```

This lock supersedes surface-first planning. Browser, code, channels, workers,
memory, and external APIs are now treated as organs or skills under one product
runtime objective:
...
-> RuntimeHost
-> ProductActionKernel
-> skill runtime / organ backend
-> receipt / replay / FinalGate
```

Future packs must either cut a blocker, hide an internal primitive, wire an
organ into the product spine, convert terminal failure to recovery, prove a
product route with receipts/replay, or update living audit truth.
...
`POWER_CLEANUP_PACK_9_MODEL_LED_PRODUCT_ACTIONKERNEL_MULTI_SKILL_TASK_LOOP_V1` adds the missing product loop layer above those one-action routes. The model can now choose multiple safe product skills in sequence, Sentinel creates a RuntimeHost mission for each material action, ProductActionKernel owns receipts and FinalGate proof, and later turns receive prior product receipt truth instead of falling back to a local-only ActionKernel loop.

`POWER_CLEANUP_PACK_10_PRODUCT_TASK_LOOP_RUNTIMEHOST_ENTRYPOINT_V1` exposes that loop through `SentinelRuntimeHost` as a bounded product entrypoint. The entrypoint frame is data-only, lists the safe model-visible skills, hides real browser/payment/credential/contact paths, and the entrypoint method calls Pack 9 rather than a local shortcut.
...
Pack 6 makes the product-spine ledger exportable and offline-verifiable in
local/fake mode. It does not claim external cryptographic signing yet.

```text
implementation_commit = 7bb5e4b0f6300629bbd04e345aa38efe012349ea
artifact export route = MissionWorkspaceRuntime artifact_export -> MissionArtifactBundleExporter
verifier route = MissionArtifactBundleVerifier over exported JSON only
integrity model = local hash-chain / external_signature_not_claimed
provider_call = no
real_browser_run = no
real_external_channel_send = no
```
...
```text
valid bundles accepted
missing ProductActionKernel receipt rejected
FinalGate receipt mismatch rejected
worker authority expansion rejected
replay side-effect deltas rejected
raw provider / reasoning / DOM / cookie / session / profile markers rejected
```

Pack 6 changes the active next proof:
