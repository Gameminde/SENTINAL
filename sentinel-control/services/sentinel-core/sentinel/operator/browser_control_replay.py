...
class BrowserControlReplayView(SentinelModel):
    mission_id: str
    ...
    @classmethod
    def from_store(cls, store: object, *, mission_id: str) -> "BrowserControlReplayView":
...
def _artifact_counts(mission_dir: Path) -> dict[str, int]:
    root = mission_dir / "browser_control"
    return {
...
def _latest_browser_state_hash(mission_dir: Path) -> str | None:
    root = mission_dir / "browser_control" / "receipts"
    if not root.exists():
        return None
    state_hashes: list[str] = []
    for path in sorted(root.glob("*.json")):
        payload = json.loads(path.read_text(encoding="utf-8"))
        for key in ("after_state_hash", "page_state_hash"):
...
__all__ = ["BrowserControlReplayView"]
