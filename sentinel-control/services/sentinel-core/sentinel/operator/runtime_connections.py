            ...
            sorted(
                (
                    ...
                    RuntimeConnectionProfile(
                        ...
                        display_name="Workspace read-only product skills",
                        runtime_generation="product_action_kernel",
                        authoritative_route=RuntimeConnectionRoute.LOCAL_GOVERNED_SURFACE,
                        maturity=RuntimeConnectionMaturity.LOCAL_ONLY,
                        owner_module="sentinel.operator.workspace_readonly_runtime",
                        owner_symbol="WorkspaceReadOnlyRuntime",
                        adapter_id="product_action_kernel_adapter",
                        supported_operations=("list", "read", "search"),
                        tool_registry_refs=("workspace_readonly",),
                        organ_registry_refs=("workspace_readonly_backend",),
                        authority_requirement="MissionAuthorityEnvelope must grant workspace.list, workspace.read, or workspace.search and the workspace path.",
                        authority_actions=("workspace.list", "workspace.read", "workspace.search"),
                        receipt_contract="ProductActionKernelReceipt + WorkspaceReadOnlyReceipt",
                        finalgate_contract="ProductActionKernelFinalGateCertificate",
                        replay_adapter="ProductActionKernelTaskLoopReplay no-reread/no-research",
                        production_reachable=True,
                        limitations=(
                            "read-only workspace files only; resolved paths must remain inside workspace; no network, shell, write, or trust-kernel mutation",
                        ),
                        test_refs=("tests/operator/test_sentinel_dev_max_power_canonical_core_v1.py",),
                    ),
                    RuntimeConnectionProfile(
