...
def _scan_forbidden_payload(payload: Any, path: str = "$") -> list[str]:
    ...
    if isinstance(payload, dict):
        for key, value in payload.items():
            ...
            if normalized in _FORBIDDEN_RETRIEVAL_KEYS and _truthy_payload(value):
...
def _contains_forbidden_text(value: str) -> bool:
    ...
    if _SECRET_LIKE_PATTERN.search(value):
        return True
    return any(marker in lowered for marker in _FORBIDDEN_RETRIEVAL_TEXT)
