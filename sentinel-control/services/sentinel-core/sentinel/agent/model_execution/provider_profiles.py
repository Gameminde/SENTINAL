...
def _default_entries() -> list[ProviderCatalogEntry]:
    return [
        ...
        _entry(
            ...
            env_var="TOKENROUTER_API_KEY",
            supported_models=["qwen/qwen3.8-max-free"],
            status=ProviderCatalogStatus.DIAGNOSTIC,
            real_status=ProviderRealTestStatusKind.DIAGNOSTIC_ONLY,
            diagnostic_outcomes=["PRODUCT_ROUTE_NOT_YET_RUN"],
            docs=["https://api.tokenrouter.com/v1"],
            capability=ProviderCapabilityFlags(
                chat=True,
                streaming=False,
                json_mode=False,
            ...
            recommendation=ProviderRecommendation(
                recommended_for=["explicit free Qwen product-route experiments", "C6 public product retry after OpenCode transport block"],
                avoid_for=["silent fallback", "unlisted paid TokenRouter model routing"],
                cost_class="free",
                reliability_class="diagnostic",
                notes=[
                    "Use exact free model id qwen/qwen3.8-max-free.",
                    "TokenRouter advertises OpenAI-compatible chat completions at /v1/chat/completions.",
                ],
            ),
        ),
        _entry(
